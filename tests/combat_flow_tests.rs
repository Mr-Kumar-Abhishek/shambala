...
use shambala::game::scene::{SceneManager, Scene};
...
fn test_game_engine_lifecycle() {
    ...
    assert!(engine.assets.textures.len() > 0);
    assert!(engine.assets.is_loaded("player_TwinBlade") == false); // Not loaded yet, just registered
...
fn test_area_exploration() {
    ...
    let area = AreaEntity::create_field(
