...
impl ExplorationSystem {
    ...
    pub fn check_enemy_proximity(
        ...
        enemies
            .iter()
            .enumerate()
            .filter(|(_, (_, pos))| PhysicsSystem::distance_between(player_pos, pos) <= aggro_range)
            .map(|(i, _)| i)
            .collect()
    }
    ...
    pub fn check_area_exit(pos: &Position, area: &Area) -> Option<(usize, usize)> {
        if let Some(ref generated) = area.generated_area {
            ...
            if tile_x < area.width
                && tile_y < area.height
                && tile_x == generated.exit.0
                && tile_y == generated.exit.1
            {
...
mod tests {
    ...
    fn test_enemy_proximity() {
        ...
        let enemies = vec![
            (
                Enemy::new(crate::components::enemy::EnemyType::Goblin),
