...
impl PlayerEntity {
    pub fn create(
        ...
        let renderable = Renderable::new(&format!("player_{:?}", class), RenderLayer::Characters);
...
mod tests {
    ...
    fn test_create_twin_blade() {
        let (player, stats, pos, _, skills, _, _, _) =
            PlayerEntity::create("Kite", Class::TwinBlade);
    ...
    fn test_create_wavemaster() {
        let (player, stats, _, _, skills, _, _, _) =
            PlayerEntity::create("Misty", Class::Wavemaster);
